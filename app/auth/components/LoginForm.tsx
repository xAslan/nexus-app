...
const LoginForm = (props: LoginFormProps) => {
  ...
  const handleSubmit = async (values) => {
    try {
      const [email, password] = [values.email, values.password]
      await loginMutation({ email: email.trim(), password: password.trim() })
      props.onSuccess && props.onSuccess()
    } catch (error) {
      if (error.name === "AuthenticationError") {
        setShowToastr(true)
        setMessage({ error: "Invalid authentication credentials" })
      } else {
        setShowToastr(true)
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div className="mt-6">
            ...
            <div className="flex items-center justify-between mt-5">
              <div className="flex items-center">
                <label htmlFor="remember_me" className="ml-2 block text-sm text-gray-900">
                  Don't have a Nexus account yet?
                </label>
              </div>

              <div className="text-md">
                <Link href="/signup">
                  <a className="font-bold text-green-600 underline hover:text-green-500">
                    Sign Up!
                  </a>
                </Link>
              </div>
